...
本项目的侧重点是后一个问题之前的基础：**在 Planner 内把字段关系算出来，并把结果而不是 Planner 私有对象交给 listener。** 这参考了上述工作，但不声称上游已经接受本方案。
...
第一阶段计算值依赖。对 `price * quantity`，`price` 和 `quantity` 都是输出值的输入；对 `CAST(price AS DECIMAL)`，输入仍是 `price`，但增加 `CAST` 处理标签；对 `1`，没有输入字段，来源是 `CONSTANT`。这里的输入字段来自 `RexInputRef`，不是重新扫描 SQL 文本。
...
第三阶段才把字段关系绑定到 sink。优化器可能复制、重排或复用 relational root；如果在逻辑 root 上算完就按 dataset 名称写入事件，两个 sink 之间会发生串边。绑定阶段必须同时使用 root slot、sink identity 和输出 schema，把同一份关系定位到实际执行的 sink 字段。
