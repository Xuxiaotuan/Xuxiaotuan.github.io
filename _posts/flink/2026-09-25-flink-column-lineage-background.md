...
本文讨论一个个人分支上的 Flink 2.4 实现探索：在原有表级 Job Lineage 基础上，增加 Planner 生成的 column lineage，并把关系交给远端 listener。这里的“原生”是指关系在 Flink 规划与提交链路内生成和交付，不表示官方发行版已经提供这项能力。

文章分四部分展开。先用一条聚合 SQL 说明表级关系缺少什么，再看社区已经提供的接口和仍待解决的问题。第三部分进入本方案：先看整体架构，再依次解释字段关系的计算、数据模型、sink 绑定和跨进程交付。最后讨论保存计划、失败处理与支持边界。这样，后面的每个设计细节都可以回到同一条链路中理解。

读者可以把下面的章节看成一张设计地图：第一部分说明问题从哪里来；第二部分说明社区已有的事件出口和本方案为什么选择 Planner；第三部分说明关系怎样被计算、组织、绑定并交付；第四部分检查直接提交、保存计划和失败状态，避免把实验结果扩大成未覆盖的 SQL 能力。
...
本文重点保留字段依赖及处理类别，不把完整计算公式的还原作为当前输出契约。
...
FLIP-314 还讨论过一个现实约束：如果把较大的血缘信息直接放进 JobGraph，可能增加提交载荷和运行端处理成本，因此首阶段采用客户端与运行端分别上报的方向，并把 column lineage 和后续合并留给后续设计。本文探索的 JobGraph 载荷路径正好落在这个已被提出、但仍需定义协议和失败边界的后续问题上。
...
本项目的侧重点是把已有的字段分析能力接进作业链路：**在 Planner 内把字段关系算出来，并把结果而不是 Planner 私有对象交给 listener。** 这参考了已有工程和上游讨论，但不声称上游已经接受本方案。

## 三、方案设计：从 Planner 计算到远端交付

社区接口提供了事件出口，接下来需要决定字段关系在哪一层生成。这个选择会影响后面的数据模型、优化绑定和传输协议。先明确职责，再沿整体链路展开。
...
本项目选择第二种。它没有测量 Planner 增量耗时或 lineage payload 大小，因此本文不提供性能数字；这些应在独立基准中测量，而不是从功能测试推断。
...
第一阶段计算值依赖。对 `price * quantity`，`price` 和 `quantity` 都是输出值的输入；对 `CAST(price AS DECIMAL)`，输入仍是 `price`，但增加 `CAST` 处理标签；对 `1`，没有输入字段，来源是 `CONSTANT`。这里的输入字段来自 `RexInputRef`，不是重新扫描 SQL 文本。原项目已经展示了从计划提取字段来源的可行性；本文的新增问题是如何把这份结果纳入 Flink 作业生命周期。
...
直接执行从当前 Planner 得到关系；Compiled Plan restore 需要从保存的计划恢复关系。在本文讨论的字段血缘语义下，两条路径的关系应一致。验证时要比较最终事件，而不是只比较作业是否成功。
